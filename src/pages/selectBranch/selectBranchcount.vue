    ...
    <Layout style="height: 100vh;">
      <Sider :width="251" style="background: #343957;">
        ...
        <Menu width="100%" :active-name="fullPath" :open-names="[openname]" accordion>
          ...
          <MenuItem name="onlineentirety" to="/selectBranchCount/count">
            <Icon type="ios-paper" />
            <span>计票页面</span>
          </MenuItem>

          <MenuItem name="offNetworkentirety" to="/selectBranchCount/offNetwork">
            <Icon type="ios-people" />
            <span>线下投票</span>
          </MenuItem>

          <MenuItem name="voteentirety" to="/selectBranchCount/vote">
            <Icon type="md-person" />
            <span>线上投票</span>
          </MenuItem>
        </Menu>
...
export default {
  ...
  computed: {
    fullPath() {
      if (this.$route.path == '/selectBranchCount/count') {
        this.openname = 'online'
        return 'onlineentirety'
      } else if (this.$route.path == '/selectBranchCount/vote') {
        this.openname = 'vote'
        return 'voteentirety'
      }else if (this.$route.path == '/selectBranchCount/specifics/count') {
        this.openname = 'online'
        return 'onlinespecifics'
      } else if (
        this.$route.path == '/selectBranchCount/specifics/offNetwork'
      ) {
        this.openname = 'offNetwork'
        return 'offNetworkspecifics'
      } else if (this.$route.path == '/selectBranchCount/specifics/vote') {
        this.openname = 'vote'
